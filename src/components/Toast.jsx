    ...
    <div className="pointer-events-none fixed inset-x-0 top-0 z-[60] flex flex-col items-center gap-2 px-4 pt-[max(1rem,env(safe-area-inset-top))]">
      <AnimatePresence initial={false}>
        {toasts.map((t) => {
          ...
          const Icon = success ? Sparkles : CircleAlert;
          return (
            <motion.div
              key={t.id}
              layout
              ...
              className={cn(
                ...
                success ? "border-primary/40" : "border-destructive/40"
              )}
            >
              <Icon className={cn("mt-0.5 h-4 w-4 shrink-0", success ? "text-primary" : "text-destructive")} />
